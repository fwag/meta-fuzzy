...
SRC_URI = "file://example-script;subdir=${PN}-${PV} \
          file://save-status-script;subdir=${PN}-${PV} \
          file://check-status-script;subdir=${PN}-${PV} \
...
do_compile() {
   ...
   # cp example-script ${MENDER_STATE_SCRIPTS_DIR}/ArtifactInstall_Enter_00
    cp save-status-script ${MENDER_STATE_SCRIPTS_DIR}/ArtifactInstall_Leave_99
    cp check-status-script ${MENDER_STATE_SCRIPTS_DIR}/ArtifactCommit_Enter_00
